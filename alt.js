...
all = ['Agate', 'Alabaster', 'Albite', 'Almandine', 'Amazonite', 'Amethyst', 'Apatite', 'Aragonite', 'Augite', 'Aventurine', 'Azurite', 'Barite', 'Beryl', 'Biotite',  'Calcite', 'Celestite', 'Chalcedony', 'Citrine', 'Copper', 'Corundum', 'Diamond', 'Dolomite', 'Epidote', 'Fluorite', 'Galena', 'Graphite', 'Halite', 'Hematite', 'Hornblende', 'Jasper', 'Kaolinite', 'Kyanite', 'Labradorite', 'Magnetite', 'Malachite', 'Milky Quartz', 'Muscovite', 'Olivine', 'Orthoclase', 'Pyrite', 'Rock Crystal', 'Rose Quartz', 'Satin Spar', 'Selenite', 'Smoky Quartz', 'Sphalerite', 'Staurolite', 'Talc', 'Topaz', "Tourmaline", 'Ulexite']
...
function minandimage(){
    mineral = all[Math.floor(Math.random()*50)]
...
function checkans() {
    ...
    if (levenshteinDistance(guess.toLowerCase(), mineral.toLowerCase()) <= 2){
        ...
        setCookie("correct2", parseInt(getCookie("correct2"))+1, 99999)
        console.log(getCookie('answered2'))

        seconds = currentTime
      
        cmineral(getCookie('usersession'), mineral) 

        document.getElementById("answered").innerHTML = getCookie("answered2")
        document.getElementById("correct").innerHTML = getCookie("correct2")
        setCookie("answered2", parseInt(getCookie("answered2"))+1, 99999)      
...
if (getCookie("correct2") == "" || getCookie("correct2") == "NaN"){
    setCookie("correct2", "0", 99999)
    setCookie("answered2", "1", 99999)
}
else {
    document.getElementById("answered").innerHTML = getCookie("answered2")
    document.getElementById("correct").innerHTML = getCookie("correct2")
